...
- Bellek tipi kapıları da devrededir: çağıran açık `types` filtresi geçerse katı uygulanır (tipleri yazılmamış kayıtlar da elenir); geçmezse sorgudan sezilen türle yumuşak kapı çalışır (çelişkili biçimde tiplenmiş kayıtlar elenir, tipi hiç yazılmamış kayıt hiçbir koşulda elenmez). Filtre yalnız arama anında uygulanır; indeks parmak izi, frekans istatistikleri ve `strict_floor` kalibrasyonu değişmez.
- Karar: hibrit yığın (BM25 dizini `records_fts`, RRF ve semantik kanca) yalnız not düzeyi yolda çalışır; passage yolu bilinçli olarak hafif sözel blok yoludur. Tur başına 1 saniyelik kurulum bütçesi ve tek-havuz kalibrasyon varsayımı blok başına hibritlemeye izin vermez; blok aramanın hibritleştirilmesi (Faz-5) bilinçli olarak ertelendi. Tip kapıları tek politika kaynağından her iki yolda da geçerlidir.
- Not düzeyi sıralama sözleri: overlap skoru yalnız admission eşiğidir, BM25 sırası hâkimdir; BM25'te görünmeyen kayıtlar tüm BM25 vuruşlarının ardından skor ve güncellikle sıralanır; semantik kanca verildiğinde füzyon RRF (k=60) ile yapılır. Bozuk FTS sorgusu ya da patlayan semantik kanca aramayı asla kırmaz; düşüş `doctor` çıktısındaki `retrieval_error_counters` ile sayılır. `doctor` ayrıca `fts_consistency` (kayıt/indekslenmiş/yetim sayıları) ve `fts_malformed_skipped` bilgisini bilgi amaçlı bildirir; indeks tüm yazma yollarında — sync dahil — kayıtlarla aynı transaction'da güncellenir.
