...
- Not düzeyi sıralama sözleri: overlap skoru yalnız admission eşiğidir, BM25 sırası hâkimdir; BM25'te görünmeyen kayıtlar tüm BM25 vuruşlarının ardından skor ve güncellikle sıralanır; semantik kanca verildiğinde füzyon RRF (k=60) ile yapılır. Bozuk FTS sorgusu ya da patlayan semantik kanca aramayı asla kırmaz; düşüş `doctor` çıktısındaki `retrieval_error_counters` ile sayılır.
