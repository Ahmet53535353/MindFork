...
Flat scalar YAML fields are also accepted, including quoted strings, numbers and JSON inline values. Unsupported multiline, nested, anchored or ambiguous YAML produces a warning and the file is excluded; metadata is never silently discarded. Use JSON frontmatter for complex facts and lists. No task or completion status is inferred from body prose.
